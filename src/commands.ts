...
const commands: Command[] = [
		...
		async editorCallback(editor: Editor) {
			...
			try {
				new ExampleModal(
					...
					self,
					async (result) => {
						await self.textGenerator.tempalteToModal(
							self.settings,
							result.path,
							editor
						);
		...
		async editorCallback(editor: Editor) {
			...
			try {
				new SetLLM(
					...
					async (result) => {
						console.log(result)
						self.settings.selectedProvider = result;
						await self.saveSettings();
					},
					"Choose a LLM"
				).open();
...

	{
		...
		async editorCallback(editor: Editor) {
          ...
          `;
				const generatedTitle = await self.textGenerator.gen(
					prompt,
					self.settings
				);
				...
				await self.app.fileManager.renameFile(
					activeFile,
					renamedFilePath
				);
		...
		async editorCallback(editor: Editor) {
			...
			try {
				new ExampleModal(
					...
					async (result) => {
						const context =
							await self.textGenerator.contextManager.getContext(
								editor,
								true,
								result.path
							);
