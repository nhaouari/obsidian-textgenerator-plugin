...
export default function funCache<T extends Function>(func: T, options: any = { lifeTime: 0, debounceTimer: 1000, async: false }): T & { clearCache: () => void } {
    let cached: any = {
    ...
    }, options.debounceTimer || 1000) : undefined;

    const checkExpiry = () => {

        if (!firstTimeDone || (options.lifeTime !== 0 && (Math.round((Date.now() - cached.____timeOfCreation)) * 10 > options.lifeTime))) {
